...
function SideBar() {
  ...
  const [padding, setPadding] = useState('pr-6')

  const fakeHover = (event) => {
  ...
  return (
    <>
      <nav className="flex flex-col justify-between fixed inset-0 bg-[#4a50bc] w-fit" onMouseEnter={(e) => fakeHover(e)} onMouseLeave={(e) => {fakeHover(e)}}>
